...
exports = module.exports = Rebound;
...
function Rebound(config) {
  ...
  if (!(this instanceof Rebound)) return new Rebound(config);
...
Rebound.prototype.add = function(item, callback) {
  ...
  if (Array.isArray(item)) return async.map(item, this.add.bind(this), callback);
  ...
  return this.client
      ...
      .exec(function(err, results) {
        ...
        if (self.length < self.queueSize) self.length++;
